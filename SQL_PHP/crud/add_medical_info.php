...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if ($stmt->num_rows > 0) {
        ...
        $insert_query = "INSERT INTO patientMedicalInfo 
                         (id, gender, age, hypertension, heart_disease, ever_married, work_type, residence_type, avg_glucose_level, bmi, smoking_status) 
                         VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
        $stmt = $conn->prepare($insert_query);
        $stmt->bind_param("isiissssdds", $user_id, $gender, $age, $hypertension, $heart_disease, 
                                           $ever_married, $work_type, $residence_type, $avg_glucose_level, $bmi, $smoking_status);
    }

    if ($stmt->execute()) {
        header("Location: ../../templates/patient/patient_home.php?success=1");
    } else {
